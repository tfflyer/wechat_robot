...
SWITCH_PREFIX=True
#消息前缀内容
PREFIX_CONTENT="[来自tfflyer's Jarvis的自动回复]"
#回复内容字典
REPLY_DICT={}
#延迟回复字典
DELAY_REPLY_DICT={}
    ...
    if msg['ToUserName']=='filehelper':
        ...
        if target_friend:
            ...
            if not any(nkname in s for s in Ad_replyed):
                ...
                #设置默认回复
                    REPLY_DICT[nickName]="抱歉，包工头看得紧，不让玩手机，不能及时回复，请见谅，如有急事，请电话联系TEL：13126883674(•ω•`)"
    ...
    if SWITCH_DELAY:
        while len(DELAY_REPLY_DICT)>0:
            ...
            for item in list(DELAY_REPLY_DICT.keys()):
                if SWITCH_REPLY :
                    reply_content = PREFIX_CONTENT + REPLY_DICT[item]
